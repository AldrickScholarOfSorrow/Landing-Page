...
def get_all_conflicts():
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            return [_to_dict(cur, row) for row in cur.fetchall()]
...
def create_conflict(data):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            new_conflict = _to_dict(cur, cur.fetchone())
            return new_conflict
...
def update_conflict(conflict_id, data):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            updated_conflict = _to_dict(cur, cur.fetchone())
            return updated_conflict

def get_conflict_stats(): # Renomeado para corresponder ao controller
